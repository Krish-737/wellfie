...
const IndicatorModal: React.FC<{
  ...
  onPrev(): void;
  onNext(): void;
}> = ({ indicator, scan, idx, total, isMobile, onClose, onPrev, onNext }) => {
  ...
  return (
    ...
    >
      <div style={{
        background: '#fff', borderRadius: isMobile ? 16 : 20, width: '100%', maxWidth: isMobile ? '94vw' : 460,
        boxShadow: '0 32px 80px rgba(0,0,0,0.22)',
        overflow: 'hidden', animation: 'mwSlideUp .2s ease',
        maxHeight: isMobile ? '90dvh' : undefined, overflowY: isMobile ? 'auto' : undefined,
      }}>
        <div style={{
          background: `linear-gradient(135deg,${indicator.catColor}22,${indicator.catColor}0a)`,
          borderBottom: '1px solid #e2e8f0', padding: isMobile ? '14px 14px 12px' : '20px 20px 16px',
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 12 }}>
              <div style={{
                ...
                <IndicatorIcon id={indicator.id} color={indicator.catColor} size={isMobile ? 18 : 24} />
              </div>
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: isMobile ? 16 : 18, fontWeight: 800, color: '#0f172a' }}>{indicator.label}</div>
                <MetricDisclaimerBadges badges={disclaimerBadges} className="mt-2" />
              </div>
            </div>
            <button type="button" onClick={onClose} style={{ background: '#f1f5f9', border: 'none', borderRadius: 8, padding: '6px 7px', cursor: 'pointer', color: '#64748b', display: 'flex', lineHeight: 1, flexShrink: 0 }}>
              <CloseIcon />
            </button>
          </div>

          <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 12, marginTop: isMobile ? 12 : 16, flexWrap: 'wrap' }}>
            <div style={{ fontSize: isMobile ? 28 : 34, fontWeight: 900, color: '#0f172a', lineHeight: 1 }}>
        ...
        <div style={{ padding: isMobile ? '14px 14px 16px' : '18px 20px 20px' }}>
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.05em', color: '#0f172a', marginBottom: 6 }}>
              What this measures
            </div>
            <p style={{ fontSize: isMobile ? 13 : 14, color: '#334155', lineHeight: 1.68, margin: 0 }}>{indicator.whatItMeans}</p>
          </div>

          <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 12, padding: isMobile ? '10px 12px' : '12px 14px', marginBottom: 12 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#92400e', marginBottom: 5 }}>When to take action</div>
            <p style={{ fontSize: isMobile ? 12 : 13, color: '#78350f', lineHeight: 1.6, margin: 0 }}>{indicator.whenToAct}</p>
          </div>
        ...
        <div style={{ borderTop: '1px solid #f1f5f9', padding: isMobile ? '8px 14px' : '10px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8fafc' }}>
          <button type="button" onClick={onPrev} disabled={idx === 0}
            style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: '1px solid #e2e8f0', borderRadius: 8, padding: isMobile ? '5px 10px' : '6px 12px', cursor: idx > 0 ? 'pointer' : 'default', opacity: idx > 0 ? 1 : 0.3, fontSize: 12, fontWeight: 600, color: '#374151', fontFamily: 'inherit' }}>
            <ChevronLeftIcon /> Prev
          </button>
          <span style={{ fontSize: 12, color: '#94a3b8' }}>{idx + 1} / {total}</span>
          <button type="button" onClick={onNext} disabled={idx === total - 1}
            style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: '1px solid #e2e8f0', borderRadius: 8, padding: isMobile ? '5px 10px' : '6px 12px', cursor: idx < total - 1 ? 'pointer' : 'default', opacity: idx < total - 1 ? 1 : 0.3, fontSize: 12, fontWeight: 600, color: '#374151', fontFamily: 'inherit' }}>
