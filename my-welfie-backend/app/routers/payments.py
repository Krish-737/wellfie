...
async def stripe_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    This is the ONLY reliable place to fulfil orders — never trust the
    success redirect URL for payment confirmation.

    Stripe signs every webhook with STRIPE_WEBHOOK_SECRET. We verify the
    signature before processing to prevent spoofed events.

    Handled events:
      - checkout.session.completed  →  credit scans to the user

    All other events return 200 immediately (Stripe requires a 200 to stop retrying).
    """
